...
bool LightSerialController::open(const std::string &port_name, int baud_rate)
{
    if (serial_->isOpen())
    {
        SPDLOG_WARN("Light serial already open on {}", port_name);
        return true;
    }
    ...
    {
        SPDLOG_ERROR("Light serial open failed: {} - {}",
                     port_name, serial_->errorString().toStdString());
        return false;
    }

    SPDLOG_INFO("Light serial opened: {} @{}", port_name, baud_rate);
    return true;
}
...
void LightSerialController::openChannel(int channel)
{
    ...
    sendMessage(cmd);
    SPDLOG_INFO("Light CH{} opened", channel);
}
...
void LightSerialController::closeChannel(int channel)
{
    ...
    sendMessage(cmd);
    SPDLOG_INFO("Light CH{} closed", channel);
}
...
void LightSerialController::setLuminance(int channel, int luminance)
{
    ...
    sendMessage(cmd);
    SPDLOG_INFO("Light CH{} luminance set to {}", channel, luminance);
}
...
void LightSerialController::sendMessage(const std::string &msg)
{
    if (!isOpen())
    {
        SPDLOG_WARN("Light serial not open, cannot send");
        return;
    }
...
void LightSerialController::sendHex(const QByteArray &data)
{
    if (!isOpen())
    {
        SPDLOG_WARN("Light serial not open, cannot send hex");
        return;
    }
    serial_->write(data);
}