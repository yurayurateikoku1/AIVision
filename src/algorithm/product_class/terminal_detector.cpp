...
void TerminalDetector::detect(NodeContext &ctx)
{
    ...
    // 转换为 TerminalResult
    auto &terminal_results = ctx.result.terminal_results;
    ...
    bool pass = (expected <= 0) || (static_cast<int>(terminal_results.size()) == expected);
    if (pass && expected_parts > 0)
    {
        for (const auto &tr : terminal_results)
        {
    ...
    if (!pass)
    {
        spdlog::warn("Terminal detect: expected {} terminals with {} parts each, got {} terminals",
                     expected, terminal_param_.parts_per_terminal, terminal_results.size());
    }
}
