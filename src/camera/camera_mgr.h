...
class CameraMgr : public QObject
{
    Q_OBJECT
public:
    explicit CameraMgr(QObject *parent = nullptr);
    ~CameraMgr();
    ...
    bool addCamera(const CameraControlParam &camera_param);

    /// @brief 添加相机（直接传入设备信息，免二次枚举）
    bool addCamera(const CameraControlParam &camera_param, MV_CC_DEVICE_INFO *dev_info);

    void removeCamera(const std::string &name);
    InterfaceCamera *getCamera(const std::string &name);
...
private:
    std::map<std::string, std::unique_ptr<InterfaceCamera>> cameras_; // name → 相机实例
